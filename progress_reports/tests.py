...
class ProgressReportsViewTest(TestCase):
    ...
    def test_progress_reports_form_view(self):
        ...
        # Debug prints
        completed_work_count = CompletedWork.objects.filter(progress_report_model=progress_report).count()
        print(f'Number of CompletedWork instances for ProgressReport {progress_report.id}: {completed_work_count}')

        assert completed_work_count == 1, "More than one CompletedWork instance found"

        completed_works = CompletedWork.objects.filter(progress_report_model=progress_report)
        if completed_works:
            completed_work = completed_works.first()
